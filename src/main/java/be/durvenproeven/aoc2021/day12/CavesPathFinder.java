...
public class CavesPathFinder {
	...
	public List<List<String>> getPaths1(String from, String to){
		List<CavesPath> startingCavePaths = connections.stream().filter(c -> c.contains(from))
				.map(c -> new CavesPath(from, to, c))
				.toList();

		List<CavesPath> res= getPaths1(startingCavePaths, new ArrayList<>());
		return res.stream().map(CavesPath::getConnectionStrings).toList();
	}

	private List<CavesPath> getPaths1(List<CavesPath> paths, List<CavesPath> alreadyFound) {
		if (paths.isEmpty()){
			return alreadyFound;
		}
		Map<Boolean, List<CavesPath>> collect = paths.stream()
				.flatMap(path -> connections.stream().flatMap(con -> path.addConnectionExtended(con).stream()))
				.collect(Collectors.groupingBy(CavesPath::isFinished));

		alreadyFound.addAll(collect.getOrDefault(true, emptyList()));
		return getPaths1(collect.getOrDefault(false, emptyList()), alreadyFound);
	}
}
