...
class GridReactorTest {
	...
	@Test
	void actualTest_Part2() {
		GridReactor gridReactor = new GridReactor(50);
		LineResolver.getStringStreamOfFile("day22GridReactor.txt")
				.map(this::getOperationWithoutLimit)
				.forEach(c -> c.accept(gridReactor));

		assertThat(gridReactor.getNrOfCubesOn()).isEqualTo(1263946820845866L);
	...
	void partTwo() {
		...
		assertThat(gridReactor.getNrOfCubesOn()).isEqualTo(2758514936282235L);
	...
	public Consumer<GridReactor> getOperationWithoutLimit(String s) {
		String coordinatesPart = StringUtils.substringAfter(s, " ");
		String[] split = coordinatesPart.split(",");
		List<Integer> firstNumberList = Arrays.stream(split)
				.map(s1 -> StringUtils.substringBetween(s1, "=", ".."))
		...
		if (s.startsWith("on")) {
			return gridReactor -> gridReactor.turnOnWithoutLimit(firstCoordinates, secondCoordinates);
		}
		return gridReactor -> gridReactor.turnOffWithoutLimit(firstCoordinates, secondCoordinates);
	}
